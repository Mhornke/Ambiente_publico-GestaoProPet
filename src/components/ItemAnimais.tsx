...
export function ItemAnimais({ data }: { data: AnimalI }) {
    ...
    return (
        <div className="max-w-sm bg-gray-200 border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
            <Link href={`/detalhes?animalId=${data.id}`}>
            ...
            <div className="p-5">
                ...
                <Link href={`/detalhes?animalId=${data.id}&animalNome=${encodeURIComponent(data.nome)}`} className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                    Conhecer melhor
